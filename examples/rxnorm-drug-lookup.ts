...
async function main(): Promise<void> {
  ...
  if (rxcui) {
    const props = await rxnorm.properties(rxcui);
    ...
    const related = await rxnorm.relatedByType(rxcui, ['BN']);
    for (const concept of related.slice(0, 5)) {
      console.log(`  ${concept.rxcui}: ${concept.name} (${concept.tty})`);
    }

    console.log('\nNDC codes:\n');

    const ndcs = await rxnorm.ndcByRxcui(rxcui);
