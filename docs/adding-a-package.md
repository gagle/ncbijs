...
Configure OIDC trusted publishing so the release workflow can publish the package without tokens:

1. Build the package: `pnpm nx run @ncbijs/{name}:build`
2. Publish the initial version: `cd packages/{name}/dist && npm publish --access public`
3. Configure OIDC trust: `pnpm npm-trust:setup`
