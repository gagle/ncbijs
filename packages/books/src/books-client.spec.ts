...
import { BooksHttpError, fetchJson } from './books-client';
...
function createConfig(overrides?: Partial<BooksClientConfig>): BooksClientConfig {
  return {
    maxRetries: 3,
...
describe('fetchJson', () => {
  ...
  it('should use BooksHttpError via createError factory', async () => {
    mockedFetchWithRetry.mockResolvedValue(mockResponse({}));
    await fetchJson('https://api.example.com/test', createConfig());
    ...
    expect(error).toBeInstanceOf(BooksHttpError);
    expect(error).toBeInstanceOf(HttpRetryError);
    expect((error as BooksHttpError).status).toBe(400);
    expect((error as BooksHttpError).body).toBe('Bad Request');
    expect(error.name).toBe('BooksHttpError');
    expect(error.message).toBe('NCBI E-utilities API (Books) returned status 400');
  });
});
