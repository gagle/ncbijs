...
import { SnpHttpError, fetchJson } from './snp-client';
...
function createConfig(overrides?: Partial<SnpClientConfig>): SnpClientConfig {
  return {
    maxRetries: 3,
...
describe('fetchJson', () => {
  ...
  it('should use SnpHttpError via createError factory', async () => {
    mockedFetchWithRetry.mockResolvedValue(mockResponse({}));
    await fetchJson('https://api.example.com/test', createConfig());
    ...
    expect(error).toBeInstanceOf(SnpHttpError);
    expect(error).toBeInstanceOf(HttpRetryError);
    expect((error as SnpHttpError).status).toBe(400);
    expect((error as SnpHttpError).body).toBe('Bad Request');
    expect(error.name).toBe('SnpHttpError');
    expect(error.message).toBe('NCBI Variation API returned status 400');
  });
});
