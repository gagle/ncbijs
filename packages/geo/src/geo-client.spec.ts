...
import { GeoHttpError, fetchJson } from './geo-client';
...
function createConfig(overrides?: Partial<GeoClientConfig>): GeoClientConfig {
  return {
    maxRetries: 3,
...
describe('fetchJson', () => {
  ...
  it('should use GeoHttpError via createError factory', async () => {
    mockedFetchWithRetry.mockResolvedValue(mockResponse({}));
    await fetchJson('https://api.example.com/test', createConfig());
    ...
    expect(error).toBeInstanceOf(GeoHttpError);
    expect(error).toBeInstanceOf(HttpRetryError);
    expect((error as GeoHttpError).status).toBe(400);
    expect((error as GeoHttpError).body).toBe('Bad Request');
    expect(error.name).toBe('GeoHttpError');
    expect(error.message).toBe('NCBI E-utilities API (GEO) returned status 400');
  });
});
