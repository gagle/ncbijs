...
import { ClinVarHttpError, fetchJson } from './clinvar-client';
...
function createConfig(overrides?: Partial<ClinVarClientConfig>): ClinVarClientConfig {
  return {
    maxRetries: 3,
...
describe('fetchJson', () => {
  ...
  it('should use ClinVarHttpError via createError factory', async () => {
    mockedFetchWithRetry.mockResolvedValue(mockResponse({}));
    await fetchJson('https://api.example.com/test', createConfig());
    ...
    expect(error).toBeInstanceOf(ClinVarHttpError);
    expect(error).toBeInstanceOf(HttpRetryError);
    expect((error as ClinVarHttpError).status).toBe(400);
    expect((error as ClinVarHttpError).body).toBe('Bad Request');
    expect(error.name).toBe('ClinVarHttpError');
    expect(error.message).toBe('NCBI E-utilities API returned status 400');
  });
});
