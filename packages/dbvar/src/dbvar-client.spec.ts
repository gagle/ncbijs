...
import { DbVarHttpError, fetchJson } from './dbvar-client';
...
function createConfig(overrides?: Partial<DbVarClientConfig>): DbVarClientConfig {
  return {
    maxRetries: 3,
...
describe('fetchJson', () => {
  ...
  it('should use DbVarHttpError via createError factory', async () => {
    mockedFetchWithRetry.mockResolvedValue(mockResponse({}));
    await fetchJson('https://api.example.com/test', createConfig());
    ...
    expect(error).toBeInstanceOf(DbVarHttpError);
    expect(error).toBeInstanceOf(HttpRetryError);
    expect((error as DbVarHttpError).status).toBe(400);
    expect((error as DbVarHttpError).body).toBe('Bad Request');
    expect(error.name).toBe('DbVarHttpError');
    expect(error.message).toBe('NCBI E-utilities API (dbVar) returned status 400');
  });
});
