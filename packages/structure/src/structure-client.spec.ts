...
import { StructureHttpError, fetchJson } from './structure-client';
...
function createConfig(overrides?: Partial<StructureClientConfig>): StructureClientConfig {
  return {
    maxRetries: 3,
...
describe('fetchJson', () => {
  ...
  it('should use StructureHttpError via createError factory', async () => {
    mockedFetchWithRetry.mockResolvedValue(mockResponse({}));
    await fetchJson('https://api.example.com/test', createConfig());
    ...
    expect(error).toBeInstanceOf(StructureHttpError);
    expect(error).toBeInstanceOf(HttpRetryError);
    expect((error as StructureHttpError).status).toBe(400);
    expect((error as StructureHttpError).body).toBe('Bad Request');
    expect(error.name).toBe('StructureHttpError');
    expect(error.message).toBe('NCBI E-utilities API (Structure) returned status 400');
  });
});
