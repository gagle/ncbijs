...
import { SraHttpError, fetchJson } from './sra-client';
...
function createConfig(overrides?: Partial<SraClientConfig>): SraClientConfig {
  return {
    maxRetries: 3,
...
describe('fetchJson', () => {
  ...
  it('should use SraHttpError via createError factory', async () => {
    mockedFetchWithRetry.mockResolvedValue(mockResponse({}));
    await fetchJson('https://api.example.com/test', createConfig());
    ...
    expect(error).toBeInstanceOf(SraHttpError);
    expect(error).toBeInstanceOf(HttpRetryError);
    expect((error as SraHttpError).status).toBe(400);
    expect((error as SraHttpError).body).toBe('Bad Request');
    expect(error.name).toBe('SraHttpError');
    expect(error.message).toBe('NCBI E-utilities API (SRA) returned status 400');
  });
});
